...
TRANSFER_PRIMITIVES: Final[tuple[TransferBaseline, ...]] = ("bilinear", "operator")
COARSENING_PRIMITIVES: Final[tuple[CoarseningKind, ...]] = (
    "full",
    "semi_x",
    "semi_y",
    "alternating",
...
# The full primitive Cartesian product is exposed for exhaustive/Pareto studies.
# The learned selector intentionally uses a smaller, interpretable candidate set
# so oracle-label generation does not explode combinatorially.
ALL_STRATEGIES: Final[tuple[MGStrategy, ...]] = tuple(
    MGStrategy(f"{smoother}_{transfer}_{coarsening}", smoother, transfer, coarsening)
    for smoother, transfer, coarsening in product(
        SMOOTHER_PRIMITIVES, TRANSFER_PRIMITIVES, COARSENING_PRIMITIVES
    )
)

STRATEGIES: Final[tuple[MGStrategy, ...]] = (
    MGStrategy("jacobi_bilinear_full", "jacobi", "bilinear", "full"),
    MGStrategy("chebyshev_bilinear_full", "chebyshev", "bilinear", "full"),
    MGStrategy("line_diag45_bilinear_full", "line_diag45", "bilinear", "full"),
    ...
    MGStrategy("chebyshev_operator_full", "chebyshev", "operator", "full"),
    MGStrategy("line_diag45_operator_full", "line_diag45", "operator", "full"),
    MGStrategy("line_x_operator_full", "line_x", "operator", "full"),
    MGStrategy("line_y_operator_full", "line_y", "operator", "full"),
    ...
    MGStrategy("chebyshev_operator_alternating", "chebyshev", "operator", "alternating"),
)
STRATEGY_BY_NAME: Final[dict[str, MGStrategy]] = {item.name: item for item in STRATEGIES}
ALL_STRATEGY_BY_NAME: Final[dict[str, MGStrategy]] = {
    item.name: item for item in ALL_STRATEGIES
}
DEFAULT_STRATEGY_NAME: Final[str] = "line_alt_bilinear_full"
N_OPERATOR_GLOBAL_FEATURES: Final[int] = 18
N_GLOBAL_FEATURES: Final[int] = N_OPERATOR_GLOBAL_FEATURES

# Needed to map bundled v3 checkpoints into the expanded v4 selector head.
V3_STRATEGY_NAMES: Final[tuple[str, ...]] = (
    "jacobi_bilinear_full",
    "line_x_bilinear_full",
    "line_y_bilinear_full",
    "line_alt_bilinear_full",
    "jacobi_operator_full",
    "line_alt_operator_full",
    "line_x_operator_semi_y",
    "line_y_operator_semi_x",
    "line_alt_operator_alternating",
)
...
def get_strategy(name: str) -> MGStrategy:
    ...
    if name in ALL_STRATEGY_BY_NAME:
        return ALL_STRATEGY_BY_NAME[name]
...
def global_context_features(
    ...
    """Return the 18 operator/grid features used by the global selector."""
    ...
    result = np.concatenate((stencil_means, diag_stats, directional, grid)).astype(
        np.float32
    )
...
def global_context_tensor(
    ...
    *,
    ...
    return torch.as_tensor(
        global_context_features(a, shape),
        device=device,
        dtype=dtype,
    ).unsqueeze(0)
